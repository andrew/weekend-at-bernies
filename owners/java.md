...
For the remediation taxonomy in [`../findings/README.md`](../findings/README.md), the maven owner picture explains the 58% switch share, the highest of any ecosystem. Most maven bernies are old coordinates left behind by namespace migrations (javax → jakarta, JUnit 4 → 5, Log4j 1 → 2) and the action is unambiguously switch to the named successor, with no conversation needed because the original org account is wound down. The remaining cases tend to be small foundation projects where the right action is also switch (springfox → springdoc, powermock → mockito inline). qos-ch is the one place where the per-package owner data adds something: the slf4j freeze and the logback activity sit under the same single-person org, so any outreach should distinguish between the two halves.

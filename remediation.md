...
## process
  ...
  3. `situate.rb` does the heuristic pre-fill. inlineable if `code_loc < 300 AND complexity < 50 AND runtime_deps = 0 AND has_native = 0`. few-large if `dependent_packages < 20 AND top1_share > 0.5` (threshold is a starting guess, tune against the rubygems set). broad if `dependent_packages > 200`. kitchen-sink if `entry_points > 10 AND code_loc > 3000`. eol_direct from `archived` only. `dead_transitive_count` / `max_dead_depth` from a recursive deps join. The `alternative` situation and README-based `eol_direct` are left to `llm.rb`; the regex grep in `size.rb` is passed as context, not used as a rule, because it matches docs prose ("use 127.0.0.1 instead of localhost") as often as real deprecation banners. Everything else left null.

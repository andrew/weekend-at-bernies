...
## pipeline
  ...
  * `/dependent_packages` 500s outright at ~30s on the largest go packages (`golang.org/x/sys` etc.) regardless of per_page or sort. Server-side query times out before LIMIT applies. **Blocked**: skipping `dependents.rb` for go until packages.ecosyste.ms endpoint is fixed; proceeding with situate/llm without `top1_share`/`transit_ratio`. Re-run `dependents.rb --ecosystem go` once the fix lands, then `situate.rb` to refine.
